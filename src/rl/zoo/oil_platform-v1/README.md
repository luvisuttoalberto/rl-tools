...
bound with the current tools. The shared actor's weight dimensions are independent
of capacity, but cross-capacity inference loading is not implemented.
...
Own/task features (13) + own presence (1)
...
There is no global swarm-count input to the actor. Presence masks still select
teammates for averaging and suppress absent agents' actions. The environment
retains `n_agents` for dynamics, reward scaling, logging, and visualization.

At capacity six, actor observations contain 390 values (65 per agent), privileged critic
...
The critics can still infer population from their per-agent presence flags.
Replay stores presence masks in its fixed-size observations, so a
...
MKL_NUM_THREADS=1 OMP_NUM_THREADS=1 ./build/src/rl/zoo/rl_zoo_oil_platform-v1_sac -s 4 --ee VARIABLE_SWARM_2_TO_6_NO_COUNT
```

Replace `build` if your CMake directory has a different name. Start a **new**
experiment. Both stage-1 three-agent checkpoints and earlier variable-swarm
checkpoints with the global count input are incompatible with this observation
layout. Already-running executables continue their original experiments.
...
the replay allocation is approximately **18.35 GB (17.09 GiB) per seed**, plus
...
Actor checkpoint groups are `swarm3x6` (version 3, capacity six, no global count)
and `sas`. The schema rejects stage-1 checkpoints, version-2 checkpoints with
the count input, and checkpoints built with a different capacity.
...
checkpoint loading and rejection of the old count-input schema, invariance to
replicating identical teammates, and compiled C++ exports with padded/dead actions.
