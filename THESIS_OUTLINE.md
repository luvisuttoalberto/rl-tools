...
- **[Ch. 4 / cross-refs] Deep-model learning algorithm asymmetry.** Ch. 4 and Ch. 5 name the SAC replay buffer, entropy objective, and adaptive temperature but give no update equations or hyperparameter values, whereas Ch. 3 §3.2.3 fully specifies Expected-SARSA. Once §2.1.6 (SAC mechanics) and §6.1.1 (hyperparameter table) are written, add cross-references from Ch. 4 so the deep model is specified at the same altitude. Also missing: a discussion of the choice of number of parallel environments, which is tightly interconnected with the replay buffer (fills it at a rate that scales with environment count, affecting the ratio of new to replayed transitions per update) and whose right value is not a simple pick, requires its own explanation of the tradeoffs.
...
- ~~**[Ch. 5 §5.1, §5.4] New battery discharge model.**~~ Done. Discharge now scales with $\lVert v_{t+1}-v_t\rVert$ (the per-step velocity-vector change) normalized by $2\alpha\,\texttt{MAX\_SPEED}$, rather than with a constant rate; §5.1's physical-plausibility list and §5.4's discharge paragraph and formula were rewritten accordingly, and the "velocity-dependent discharge is future work" caveat was dropped. Code: `ACCELERATION_PROPORTIONAL_DISCHARGE` (enabled by default) in `oil_platform.h`; `VELOCITY_PROPORTIONAL_DISCHARGE` (a discarded earlier variant that scaled with speed rather than with speed change) is kept disabled for ablations only and is intentionally not mentioned in the thesis text. Trained and tested by the user. Still open: if specific behavioral results/metrics from that training run should be cited in §5.4 or elsewhere, add them once identified.
- **[Ch. 6, experiments] Scripted non-learning baseline (priority).** Implement a hand-crafted baseline policy (e.g., partitioned patrol + threshold-based return-to-charge + converge-on-detection) and compare it against the learned policy on the same metrics. It directly supports the Chapter 1 claim that no explicit algorithm easily specifies this behavior, gives readers an interpretable performance floor, and is the comparison reviewers of applied MARL work most often ask for. Plan: write it with Claude in a dedicated session, then run the metric comparison.
- **[Ch. 6, experiments] MAPPO comparison (optional).** Train MAPPO on the oil platform environment (rl-tools already supports it) to substantiate the sample-efficiency motivation for SAC stated in Chapters 2 and 4. Honest-thesis consequence: if MAPPO matches or beats the multi-agent SAC, the motivation sections need rewording (feasible, since the choice was also architectural); run it only if prepared to report either outcome.
- **[Ch. 2 §2.1] MAAC motivation.** Study MAAC (Iqbal & Sha, ICML 2019) and decide the honest motivation for not building on it; two TODO comments in ch02.tex (policy-gradient subsection and "Relation to this thesis") mark where the motivation goes. Do not invent reasons before the study.
...
- **[Ch. 5 §5.10, Eq.~\ref{eq:total_reward}] Potential-based charging-station shaping.** Under testing, looks promising, not yet reflected in the text. The spatial pull toward the charger, currently described as the legacy level term inside $r_{\mathrm{chg}}$ ($0.3 \cdot u_i \cdot \mathcal{N}(d_i, \sigma_{\mathrm{chg}})$), has been replaced by a separate potential-based shaping term added directly to the total reward: $r_{\mathrm{shp}} = \sum_i \left[\gamma\,\Phi(s_i') - \Phi(s_i)\right]$, with $\Phi(s) = \beta_{\mathrm{chg}} \cdot 0.3 \cdot u(s) \cdot \mathcal{N}(d(s), \sigma_{\mathrm{chg}})$ evaluated for every alive agent every step and $\gamma = 1.0$. Because only *changes* in $\Phi$ are rewarded, an agent hovering just outside the docking radius nets ~0 instead of farming a level proximity bonus, while the gradient toward the charger is preserved. With this flag on, the legacy level term inside $r_{\mathrm{chg}}$ is skipped entirely, so $r_{\mathrm{chg}}$ itself reduces to tracking only actual charging sessions and the spatial pull moves out of $r_{\mathrm{chg}}$ into the new $r_{\mathrm{shp}}$ term in Eq.~\ref{eq:total_reward}. Code: `POTENTIAL_BASED_CHARGING_SHAPING` (default on) and `charging_shaping_potential(...)` in `operations_generic.h`; `CHARGING_SHAPING_POTENTIAL_GAMMA` sets $\gamma$. The legacy level formulation is kept, gated by the same flag, for ablations. If adopted, rewrite §5.10 to describe $\Phi$ and $r_{\mathrm{shp}}$, add $r_{\mathrm{shp}}$ to Eq.~\ref{eq:total_reward}, and confirm the exact constants against the code once training results are in.
...
- Asymmetric critic inputs build on single-agent asymmetric actor-critic (Pinto et al., RSS 2018, arXiv 1710.06542: critic trained on full simulator state including quantities the deployed robot cannot sense, actor on images); the critic in this thesis (§5.7.2) follows the same principle, receiving the full state including ground-truth disaster position and velocity (which deployed actors only estimate through stochastic detection); the non-standard element is the remaining episode time, an input that is not part of the environment state at all
...
- **FACMAC** (Peng et al., NeurIPS 2021): factored centralized critic in QMIX style with deterministic per-agent actors; handles discrete and continuous actions; bridge between the value-based and policy-gradient families
...
- **MAAC** (Iqbal & Sha, "Actor-Attention-Critic for Multi-Agent Reinforcement Learning", ICML 2019): SAC-based multi-agent actor-critic with attention over other agents' observations in the critic; off-policy → reuse experience; continuous actions; entropy-regularized objective → stable exploration; conceptually the closest prior work to this thesis; **not available in any high-performance C++ framework**; designed for competitive/mixed settings; attention adds complexity not needed in fully cooperative homogeneous settings

#### 2.1.5 Parameter sharing
- Standard technique for cooperative homogeneous MARL: all N agents share one network, differentiated by a per-agent ID input (one-hot encoding or learned embedding) [Gupta et al., 2017; Terry et al., 2020, arXiv 2005.13625]
...
**Positioning:** Existing CTDE algorithms either (a) require discrete actions (QMIX, VDN), (b) use N separate critics and no entropy regularization (MADDPG), (c) are available only in Python with no embedded deployment path (MAAC, MAPPO — for MAAC, pending the study flagged in the TODO below), or (d) support MARL but not SAC in C++ (rl-tools pre-this-work). Chapter 4 fills the gap: a multi-agent SAC with strictly privileged critic observations, implemented in rl-tools for high-performance training and embedded deployment.
...
- Recent RL coverage work typically assumes: discrete grid world with perfect sensing, reward = fraction of unique cells visited, single agent or small N without resource constraints, no dynamic events requiring response [verified candidates to cite (confirm each abstract supports the attributed claim before citing): Theile et al., "UAV Coverage Path Planning under Varying Power Constraints using Deep Reinforcement Learning" / "Multi-UAV Path Planning for Wireless Data Harvesting with Deep RL" (arXiv 2010.12461); Jonnarth et al., "Learning Coverage Paths in Unknown Environments with Deep Reinforcement Learning" (arXiv 2306.16978)]
- **What sensor-range coverage adds:** hard sensor-range cutoff enforced in the reward (an agent beyond SENSOR_RANGE = 50 m gets zero credit for a cell — physically grounded); nearest-agent assignment prevents the overlap incentive present in Gaussian formulations where two agents near the same cell each collect positive reward; battery-urgency weighting embeds resource management directly in the coverage metric; result: a single shaped reward naturally produces spatial repulsion, sensor-range awareness, and charging coordination without separate auxiliary terms for each behavior
...
- Early multi-UAV coordination was control-theoretic, not learned: decentralized nonlinear MPC for multiple autonomous helicopters (Shim, Kim & Sastry, CDC 2003)
- Recent RL-based multi-UAV search/coverage: decentralized RL for multi-target search and detection by a team of drones (arXiv 2103.09520); multi-UAV path planning for wireless data harvesting with deep RL (Theile et al., arXiv 2010.12461); confirm each abstract supports the attributed claims (discrete actions, battery model, deterministic targets) before citing
...
|---|---|---|---|
| CTDE algorithms | MADDPG, MAAC (Python); QMIX (discrete only); MAPPO in rl-tools | No multi-agent SAC with privileged asymmetric critic in C++ | Ch. 4, §4.2–4.4 |
...
- [ ] Fig. 2.2: Algorithm comparison table (visual) — rows: MADDPG, QMIX, MAPPO, MAAC, this work; columns: action space, on/off-policy, parameter sharing, privileged critic, C++ deployment
